...
export function ClinicPatientEvolutions({ patient }: { patient:ClinicPatientDetail }) {
  ...
  const load=useCallback(async () => {
    setError(""); setLoading(true); setRows([]); setDrafts([]); setCanWrite(false);
    try {
      const result=await clinicEvolutionRequest(patient.organizationPatientId);
      setRows(result.evolutions || []); setCanWrite(Boolean(result.canWrite));
  ...
  async function download(row:any) {
    ...
    try {
      ...
      const result=await clinicEvolutionRequest(patient.organizationPatientId,"GET",undefined,row.id);
      const evo=result.evolution;
      const { data:professional }=await supabase.from("professionals").select("full_name, professional_title, professional_register").eq("id",user?.id).single();
    ...
    } catch { setError("Não foi possível exportar sua evolução."); }
    finally { setBusy(false); }
  }
  return <section className="rounded-2xl border border-brand-border bg-white p-6 shadow-sm space-y-4">
    <div className="flex flex-wrap items-center justify-between gap-3"><h2 className="text-lg font-semibold text-brand-text">Minhas evoluções</h2>{canWrite && <Link to={newPath} className="btn-primary">Nova evolução</Link>}</div>
    <p className="text-sm text-brand-text-muted">Compartilhar o paciente não compartilha as evoluções. Apenas suas próprias anotações aparecem aqui.</p>
