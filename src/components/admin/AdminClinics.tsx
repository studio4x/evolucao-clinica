...
import {
  ...
  clinicPlanLabel,
  clinicRoleLabel,
} from '../../utils/clinicAdminPresentation';
...
const financialLabel = clinicFinancialStatusLabel;
...
export default function AdminClinics() {
  ...
  const filtered = useMemo(() => clinics.filter((clinic) => {
    ...
    return (!query || haystack.includes(query.toLowerCase()))
      ...
      && (featureFilter === 'all' || (featureFilter === 'enabled' ? clinic.featureEnabled : !clinic.featureEnabled))
      && (planFilter === 'all' || clinic.subscription?.planCode === planFilter);
  }), [clinics, featureFilter, financialFilter, planFilter, query, statusFilter]);
  ...
  return <div className="space-y-6">
    ...
    <div className="flex flex-col gap-3 rounded-2xl border border-brand-border/70 bg-white p-4 shadow-sm lg:flex-row lg:flex-wrap"><label className="relative min-w-[240px] flex-1"><Search size={17} className="absolute left-3 top-3 text-brand-text-muted" /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Buscar por clínica ou responsável" className="min-h-11 w-full rounded-xl border border-brand-border pl-10 pr-3 text-sm outline-none focus:border-brand-primary focus:ring-2 focus:ring-brand-primary/10" /></label><select value={statusFilter} onChange={(event) => setStatusFilter(event.target.value)} className="min-h-11 rounded-xl border border-brand-border px-3 text-sm"><option value="all">Todos os status</option><option value="active">Ativas</option><option value="pending_setup">Configuração pendente</option><option value="restricted">Restritas</option><option value="archived">Arquivadas</option></select><select value={financialFilter} onChange={(event) => setFinancialFilter(event.target.value)} className="min-h-11 rounded-xl border border-brand-border px-3 text-sm"><option value="all">Toda situação financeira</option><option value="active">Ativo</option><option value="past_due">Pagamento pendente</option><option value="unpaid">Não pago</option><option value="canceled">Cancelado</option></select><select value={featureFilter} onChange={(event) => setFeatureFilter(event.target.value)} className="min-h-11 rounded-xl border border-brand-border px-3 text-sm"><option value="all">Todos os recursos Clínica</option><option value="enabled">Piloto habilitado</option><option value="disabled">Desabilitado</option></select><select value={planFilter} onChange={(event) => setPlanFilter(event.target.value)} className="min-h-11 rounded-xl border border-brand-border px-3 text-sm"><option value="all">Todos os planos</option>{Array.from(new Set(clinics.map((clinic) => clinic.subscription?.planCode).filter(Boolean))).map((plan) => <option key={plan} value={plan}>{clinicPlanLabel[String(plan)] || String(plan)}</option>)}</select></div>
    {error && <div role="alert" className="rounded-2xl border border-red-100 bg-red-50 px-4 py-3 text-sm text-red-700">{error}</div>}
    <div className="overflow-hidden rounded-2xl border border-brand-border/70 bg-white shadow-sm">{loading ? <div className="p-10 text-center text-sm text-brand-text-muted">Carregando clínicas…</div> : filtered.length === 0 ? <div className="p-10 text-center text-sm text-brand-text-muted">Nenhuma clínica encontrada.</div> : <div className="overflow-x-auto"><table className="min-w-[1080px] w-full text-left text-sm"><thead className="bg-brand-bg/50 text-xs uppercase tracking-wide text-brand-text-muted"><tr><th className="px-5 py-4">Clínica</th><th className="px-5 py-4">Status</th><th className="px-5 py-4">Plano</th><th className="px-5 py-4">Responsável</th><th className="px-5 py-4">Membros</th><th className="px-5 py-4">Licenças</th><th className="px-5 py-4">Financeiro</th><th className="px-5 py-4">Criada em</th><th className="px-5 py-4">Ações</th></tr></thead><tbody className="divide-y divide-brand-border/60">{filtered.map((clinic) => <tr key={clinic.id} className="align-top hover:bg-brand-bg/30"><td className="px-5 py-4"><p className="font-semibold text-brand-primary">{clinic.name}</p>{clinic.tradeName && <p className="mt-0.5 text-xs text-brand-text-muted">{clinic.tradeName}</p>}<p className="mt-1 font-mono text-[10px] text-brand-text-muted">{clinic.id}</p></td><td className="px-5 py-4">{badge(clinic.operationalStatus === 'active' ? 'green' : clinic.operationalStatus === 'pending_setup' ? 'amber' : 'slate', statusLabel[clinic.operationalStatus] || clinic.operationalStatus)}<p className="mt-2 text-[11px]">{clinic.featureEnabled ? badge('green', 'Piloto habilitado') : badge('slate', 'Desabilitado')}</p></td><td className="px-5 py-4"><p className="font-medium">{clinicPlanLabel[clinic.subscription?.planCode || ''] || '—'}</p><p className="text-xs text-brand-text-muted">{clinicBillingIntervalLabel[clinic.subscription?.billingCycle || ''] || '—'}</p></td><td className="px-5 py-4"><p>{clinic.owner?.name || 'Sem proprietário identificado'}</p><p className="text-xs text-brand-text-muted">{clinic.owner?.email || '—'}</p>{clinic.ownerIntegrity !== 'OK' && <p className="mt-1 text-[11px] font-semibold text-amber-700">Integridade do proprietário: {clinic.ownerIntegrity}</p>}</td><td className="px-5 py-4"><span className="inline-flex items-center gap-1"><Users size={14} /> {clinic.members.total}</span><p className="text-xs text-brand-text-muted">{clinic.members.clinical} clínicos · {clinic.members.administrative} administrativos</p></td><td className="px-5 py-4"><p>{clinic.seatUsage.activeSeats}/{clinic.subscription?.contractedSeats || 0} ativas</p><p className="text-xs text-brand-text-muted">{clinic.seatUsage.reservedSeats} reservadas · {clinic.seatUsage.availableSeats} disponíveis</p></td><td className="px-5 py-4">{badge(clinic.subscription?.financialStatus === 'active' ? 'green' : clinic.subscription?.financialStatus === 'past_due' ? 'amber' : 'slate', financialLabel[clinic.subscription?.financialStatus || ''] || 'Sem contrato')}</td><td className="px-5 py-4 text-xs text-brand-text-muted">{new Date(clinic.createdAt).toLocaleDateString('pt-BR')}</td><td className="px-5 py-4"><button type="button" onClick={() => setSelected(clinic)} className="inline-flex min-h-10 items-center gap-1 rounded-lg border border-brand-border px-3 font-semibold text-brand-primary">Ver detalhes <ChevronDown size={15} /></button></td></tr>)}</tbody></table></div>}</div>
    {selected && <div className="fixed inset-0 z-50 flex items-end justify-center bg-slate-950/35 p-0 sm:items-center sm:p-6" role="dialog" aria-modal="true" aria-label={`Detalhes de ${selected.name}`} onClick={(event) => { if (event.target === event.currentTarget) setSelected(null); }}><div className="max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-t-3xl bg-white p-6 shadow-2xl sm:rounded-3xl"><div className="flex items-start justify-between gap-4"><div><p className="text-xs font-bold uppercase tracking-[0.12em] text-brand-primary">Detalhes da clínica</p><h2 className="mt-2 font-display text-2xl font-bold text-brand-primary">{selected.name}</h2><p className="mt-1 text-sm text-brand-text-muted">{selected.tradeName || 'Sem nome fantasia'}</p></div><button type="button" className="rounded-lg border border-brand-border p-2 text-brand-text-muted" onClick={() => setSelected(null)} aria-label="Fechar detalhes">×</button></div><div className="mt-6 grid gap-3 sm:grid-cols-2"><Info label="Status" value={statusLabel[selected.operationalStatus] || selected.operationalStatus} /><Info label="Proprietário" value={selected.owner?.name || 'Não identificado'} /><Info label="Plano / contrato" value={`${clinicPlanLabel[selected.subscription?.planCode || ''] || '—'} · ${financialLabel[selected.subscription?.financialStatus || ''] || 'Sem situação'}`} /><Info label="Situação do acesso" value={clinicEntitlementLabel[selected.subscription?.entitlementMode || 'none'] || 'Indisponível'} /><Info label="Licenças" value={`${selected.seatUsage.activeSeats} ativas · ${selected.seatUsage.reservedSeats} reservadas · ${selected.seatUsage.availableSeats} disponíveis`} /><Info label="Recurso Clínica" value={selected.featureEnabled ? 'Piloto habilitado' : 'Desabilitado'} /><Info label="Convites pendentes" value={`${selected.pendingInvitations.total} (${selected.pendingInvitations.clinicalReserved} clínicos)`} /></div><div className="mt-6 rounded-2xl border border-brand-border/70 p-4"><div className="flex items-center justify-between"><h3 className="font-semibold text-brand-primary">Membros da clínica</h3><span className="text-xs text-brand-text-muted">{selected.members.total} ativos</span></div>{selected.memberList?.length ? <div className="mt-3 divide-y divide-brand-border/60">{selected.memberList.map((member) => <div key={member.id} className="flex items-center justify-between gap-3 py-3 text-sm"><div><p className="font-medium">{member.name || 'Sem nome'}</p><p className="text-xs text-brand-text-muted">{member.email || '—'}</p></div><div className="text-right text-xs"><p>{clinicRoleLabel[member.role] || 'Profissional'}</p><p className={member.clinicalAccessEnabled ? 'text-emerald-700' : 'text-brand-text-muted'}>{member.clinicalAccessEnabled ? 'Acesso clínico' : 'Acesso administrativo'}</p><p className="text-brand-text-muted">{clinicMemberStatusLabel[member.status] || 'Removido'}</p></div></div>)}</div> : <p className="mt-3 text-sm text-brand-text-muted">Nenhum membro ativo retornado.</p>}</div><div className="mt-6 flex flex-wrap gap-3"><button type="button" className="inline-flex min-h-10 items-center gap-2 rounded-xl border border-brand-border px-4 text-sm font-semibold text-brand-primary" onClick={() => void navigator.clipboard?.writeText(selected.id)}><Copy size={15} /> Copiar ID</button><button type="button" className="min-h-10 rounded-xl bg-brand-primary px-4 text-sm font-semibold text-white" onClick={() => setSelected(null)}>Fechar</button></div></div></div>}
  </div>;
}
