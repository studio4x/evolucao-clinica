...
export default function LifecycleAdmin() {
  ...
  return <div className="space-y-6">
    ...
    {tab === 'campaigns' && <div className="space-y-6">
      ...
      {campaignTab === 'logs' && <div className="overflow-hidden rounded-xl border border-brand-border bg-white">{deliveries.length === 0 ? <EmptyState icon={ScrollText} title="Nenhum registro de envio" description="Os disparos processados pela fila aparecerão aqui." /> : <table className="w-full table-fixed text-left text-sm"><thead className="bg-brand-bg text-xs uppercase text-brand-text-muted"><tr><th className="w-[19%] p-3">Destinatário</th><th className="w-[20%] p-3">Modelo enviado</th><th className="w-[9%] p-3">Status</th><th className="w-[18%] p-3">Enviada/agendada</th><th className="w-[9%] p-3">Tentativas</th><th className="w-[17%] p-3">Motivo</th><th className="w-[8%] p-3 text-right">Ação</th></tr></thead><tbody className="divide-y divide-brand-border">{deliveries.map((item) => <tr key={item.id} className="hover:bg-brand-bg/40"><td className="p-3"><strong className="block max-w-full truncate">{item.recipient_name}</strong><span className="block max-w-full truncate text-xs text-brand-text-muted">{item.recipient_email}</span></td><td className="p-3"><strong className="block max-w-full truncate" title={item.template_name}>{item.template_name}</strong><span className="block max-w-full truncate text-xs text-brand-text-muted" title={item.template_key}>{item.template_reference}</span></td><td className="p-3"><span className={'rounded-full px-2.5 py-1 text-xs font-medium ' + (item.status === 'sent' ? 'bg-emerald-100 text-emerald-700' : item.status === 'failed' ? 'bg-red-100 text-red-700' : 'bg-slate-100 text-slate-600')}>{deliveryStatusLabel(item.status)}</span></td><td className="p-3 whitespace-nowrap">{(item.sent_at || item.scheduled_for) ? new Date(item.sent_at || item.scheduled_for).toLocaleString('pt-BR') : '—'}</td><td className="p-3">{item.attempt_count}/{item.max_attempts}</td><td className="p-3 max-w-full truncate" title={item.skip_reason || item.failure_reason || ''}>{item.skip_reason || item.failure_reason || '—'}</td><td className="p-3 text-right"><button onClick={() => void resendLifecycleDelivery(item)} disabled={resendingDispatchId === item.id} title="Reenviar este e-mail" className="btn-outline inline-flex items-center gap-1.5 p-1.5 text-xs disabled:cursor-not-allowed disabled:opacity-50">{resendingDispatchId === item.id ? <Loader2 size={14} className="animate-spin" /> : <Send size={14} />}<span className="sr-only">Reenviar</span></button></td></tr>)}</tbody></table>}</div>}
    </div>}
