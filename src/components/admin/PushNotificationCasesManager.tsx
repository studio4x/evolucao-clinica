...
import { AlertTriangle, Bell, Check, ChevronDown, Loader2, RefreshCw } from 'lucide-react';
...
export default function PushNotificationCasesManager() {
  ...
  return (
    <section className="card overflow-hidden border border-brand-border/60 bg-white shadow-sm">
      <div className={`flex flex-wrap items-start justify-between gap-3 p-5 ${isOpen ? 'border-b border-brand-border/40' : ''}`}>
        <button
          type="button"
        ...
        >
          ...
          <div className="min-w-0 flex-1">
            <div className="flex items-center gap-2">
              ...
              <ChevronDown size={18} className={`shrink-0 text-brand-text-muted transition-transform ${isOpen ? 'rotate-180' : ''}`} />
            </div>
        ...
        {isOpen && (
          <button
            ...
            className="btn-outline inline-flex items-center gap-1.5 px-3 py-2 text-xs disabled:opacity-50"
          >
