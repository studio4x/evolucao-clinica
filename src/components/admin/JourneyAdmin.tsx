...
export default function JourneyAdmin() {
  ...
  const handleJourneyPublicationAction = async (publication: JourneyWhatsAppPublication, action: 'requeue' | 'cancel' | 'reset_sent') => {
    if (action === 'reset_sent') {
      const day = publication.journey_contents?.day_number ?? '?';
      const phrase = `RESETAR DIA ${day}`;
      const confirmation = await showPrompt(`Resetar envio do Dia ${day}?\n\nEsta ação removerá o registro de entrega ao WhatsApp e permitirá que este conteúdo seja enviado novamente.\n\nSe o conteúdo continuar publicado e com horário vencido, ele poderá ser reenviado automaticamente na próxima execução do cron.\n\nA mensagem já entregue anteriormente não será apagada do WhatsApp.\n\nPara confirmar, digite: ${phrase}`, { title: 'Resetar envio', confirmLabel: 'Confirmar reset', cancelLabel: 'Cancelar', variant: 'danger', icon: 'warning', placeholder: phrase, requiredValue: phrase });
      ...
      const body = await response.json();
      if (!response.ok) { await showAlert(body.error || 'Não foi possível resetar o envio.', { title: 'Reset não realizado', variant: 'danger', icon: 'warning' }); return; }
      if (selectedJourney) { await fetchJourneyPublications(selectedJourney.id); await fetchContents(selectedJourney.id); }
      const message = body.editorialStatus === 'draft' ? 'Envio resetado. O conteúdo está em rascunho e não será enviado até ser agendado e publicado novamente.' : body.editorialStatus === 'scheduled' ? 'Envio resetado. O conteúdo será liberado somente depois da publicação programada.' : 'Envio resetado. O conteúdo poderá ser reenviado automaticamente na próxima execução da automação.';
      await showAlert(`${message}\n\nA mensagem anterior continua no WhatsApp.`, { title: 'Envio resetado', variant: 'success', icon: 'success' });
      return;
    }
