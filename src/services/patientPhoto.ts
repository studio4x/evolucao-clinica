...
export const PATIENT_PHOTO_HEIC_TYPES = ['image/heic', 'image/heif', 'image/heic-sequence', 'image/heif-sequence'];
const PATIENT_PHOTO_SOURCE_EXTENSIONS = /\.(png|jpe?g|webp|heic|heif)$/i;

export const isPatientPhotoHeic = (file: Pick<File, 'name' | 'type'>): boolean => (
  PATIENT_PHOTO_HEIC_TYPES.includes(file.type.toLowerCase())
...
export const validatePatientPhotoSource = (file: File): string | null => {
  ...
  if (!hasSupportedType && !hasSupportedExtension) {
    return 'O celular informou um formato não compatível. Selecione uma imagem JPG, PNG, WEBP ou HEIC/HEIF.';
  }
