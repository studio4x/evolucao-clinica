...
export async function fetchClinicContexts(accessToken: string): Promise<ClinicContextsPayload> {
  console.info(`[Clinic2CDiag] F1 ${JSON.stringify({ fetch_started: true })}`);
  ...
  console.info(`[Clinic2CDiag] F2/F3/F4 ${JSON.stringify({ status: response.status, error: typeof body?.error === "string" ? body.error : null, organizations_count: Array.isArray(body?.organizations) ? body.organizations.length : null })}`);
