...
import {
  isPublicAnamnesisSubmittedStatus,
  PUBLIC_ANAMNESIS_SESSION_KEY,
  resolvePublicAnamnesisLoadPlan,
} from '../utils/publicAnamnesisSession';
...
type Bootstrap = { session: string; expiresAt?: string; respondentType: 'patient' | 'responsible'; identity: { patientInitials: string; professionalFirstName: string; professionalTitle: string | null }; branding?: PublicBranding; snapshot: Snapshot; draft: Record<string, unknown>; revision: number; respondentName: string; respondentRelationship: string; status?: string };
...
export default function PublicAnamnesisForm() {
  ...
  const load = async () => {
    setLoading(true);
    try {
      const plan = resolvePublicAnamnesisLoadPlan(token, sessionStorage.getItem(PUBLIC_ANAMNESIS_SESSION_KEY));
      if (plan.mode === 'unavailable') throw new Error('unavailable');
      const response = plan.mode === 'bootstrap'
        ? await fetch('/api/public/anamnesis-link/bootstrap', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ token: plan.token }) })
        : await fetch('/api/public/anamnesis-link/form', { headers: { Authorization: `Bearer ${plan.session}` } });
      ...
      if (!response.ok) {
        ...
        if (response.status === 404 || response.status === 410) sessionStorage.removeItem(PUBLIC_ANAMNESIS_SESSION_KEY);
        throw new Error('unavailable');
      }
      const boot = payload as Bootstrap;
      if (boot.session) sessionStorage.setItem(PUBLIC_ANAMNESIS_SESSION_KEY, boot.session);
      if (plan.mode === 'bootstrap') window.history.replaceState({}, document.title, '/preencher/anamnese');
      setBranding(boot.branding || null);
      setData(boot);
      ...
      setSubmitted(isPublicAnamnesisSubmittedStatus(boot.status));
  ...
  const save = async (nextAnswers = answers, nextName = respondentName, nextRelationship = respondentRelationship) => {
    ...
    try {
      const response = await fetch('/api/public/anamnesis-link/draft', { method: 'PATCH', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${sessionStorage.getItem(PUBLIC_ANAMNESIS_SESSION_KEY) || ''}` }, body: JSON.stringify({ answers: nextAnswers, baseRevision: revision, respondentName: nextName, respondentRelationship: nextRelationship }) });
      const payload = await response.json().catch(() => ({}));
      ...
      if (payload.session) sessionStorage.setItem(PUBLIC_ANAMNESIS_SESSION_KEY, payload.session);
  ...
  const submit = async () => {
    ...
    try {
      const response = await fetch('/api/public/anamnesis-link/submit', { method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${sessionStorage.getItem(PUBLIC_ANAMNESIS_SESSION_KEY) || ''}` }, body: JSON.stringify({ answers, baseRevision: revision, respondentName, respondentRelationship, idempotencyKey: crypto.randomUUID() }) });
      const payload = await response.json().catch(() => ({}));
      ...
      setSubmitted(true); setStep(reviewStep);
  ...
  if (loading) return <PublicShell branding={branding}><div className="flex items-center justify-center gap-3 py-20 text-sm text-brand-text-muted"><Loader2 className="animate-spin" size={20} />Carregando formulário...</div></PublicShell>;
  if (message === 'submitted') return <PublicShell branding={branding}><StateCard title="Formulário já enviado" text="Este formulário já foi encaminhado ao profissional responsável." /> </PublicShell>;
  if (message === 'unavailable') return <PublicShell branding={branding}><StateCard title="Formulário indisponível" text="O link não está disponível. Verifique se ele está correto ou solicite um novo link ao profissional." /></PublicShell>;
  if (!data) return <PublicShell branding={branding}><StateCard title="Não foi possível carregar" text={message || 'Tente novamente em alguns instantes.'} /></PublicShell>;

  if (submitted) return <PublicShell branding={branding}><div className="rounded-3xl border border-emerald-200 bg-emerald-50 p-6 text-center sm:p-10"><CheckCircle2 className="mx-auto text-emerald-600" size={48} /><h1 className="mt-5 text-2xl font-bold text-emerald-950">Formulário enviado</h1><p className="mx-auto mt-3 max-w-md text-sm leading-relaxed text-emerald-900">As informações foram encaminhadas ao profissional responsável para revisão.</p></div></PublicShell>;
  ...
  return <PublicShell branding={branding}>
...
function PublicShell({ children, branding }: { children: React.ReactNode; branding?: PublicBranding | null }) {
  const [logoUnavailable, setLogoUnavailable] = useState(false);
  const showLogo = Boolean(branding?.logo) && !logoUnavailable;
  return <main className="min-h-screen bg-brand-bg px-4 py-6 sm:px-6 sm:py-10"><div className="mx-auto w-full max-w-3xl"><div className="mb-6 text-center">{showLogo ? <img src={branding?.logo} alt="Logotipo do profissional" onError={() => setLogoUnavailable(true)} className="mx-auto max-h-14 max-w-[220px] object-contain" /> : <p className="text-lg font-bold text-brand-text">Evolução Clínica</p>}<p className="mt-1 text-xs text-brand-text-muted">Preenchimento de formulário</p></div>{children}<p className="mt-6 text-center text-[11px] leading-relaxed text-brand-text-muted">Suas respostas serão enviadas ao profissional responsável para revisão.</p></div></main>;
}
