...
export default function AnamnesisModels() {
  ...
  const returnTo = requestedReturnTo && requestedReturnTo.startsWith('/painel/') && requestedReturnTo !== location.pathname
    ? requestedReturnTo
    : '/painel';
  const openBuilder = (path: string) => navigate(path, { state: { from: returnTo } });
  ...
  const renderPersonal = (template: AnamnesisTemplate) => (
    <article key={template.id} className={`${cardClass} ${template.status === 'archived' ? 'opacity-70' : ''}`}>
      ...
      <div className="mt-4 flex flex-wrap gap-2">
        <button type="button" onClick={() => openBuilder(`/painel/anamnesis/modelos/${template.id}/editar`)} className="btn-outline inline-flex items-center gap-1.5 px-3 py-2 text-xs"><Edit3 size={14} />Editar</button>
        <button type="button" onClick={() => openBuilder(`/painel/anamnesis/modelos/novo?source=${template.id}`)} className="btn-outline inline-flex items-center gap-1.5 px-3 py-2 text-xs"><Copy size={14} />Duplicar</button>
  ...
  return <div className="w-full space-y-5 pb-10">
    ...
    {loading ? <div className="card p-6 text-sm text-brand-text-muted">Carregando modelos...</div> : <>
      ...
      <section className="space-y-3"><div><h2 className="text-lg font-bold text-brand-text">Modelos recomendados</h2><p className="text-xs text-brand-text-muted">Você pode personalizar uma cópia; o modelo oficial não será alterado.</p></div><div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">{standard.map((template) => <article key={template.id} className={cardClass}><h3 className="font-bold text-brand-text">{template.name}</h3><p className="mt-1 text-xs text-brand-text-muted">Modelo oficial · versão {template.version}</p><button type="button" onClick={() => openBuilder(`/painel/anamnesis/modelos/novo?source=${template.id}`)} className="btn-outline mt-4 inline-flex items-center gap-1.5 px-3 py-2 text-xs"><Copy size={14} />Personalizar modelo</button></article>)}</div></section>
    </>}
