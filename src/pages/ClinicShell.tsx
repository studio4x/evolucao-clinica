  ...
  return <div className="space-y-6">
    ...
    {loading ? <div className="flex items-center gap-2 text-sm text-brand-text-muted"><Loader2 className="animate-spin" size={18} /> Carregando indicadores...</div> : dashboard && <>
      <section className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        {admin ? <><Card label="Pacientes ativos" value={dashboard.patients?.active || 0} detail={`${dashboard.patients?.archived || 0} arquivados`} /><Card label="Equipe ativa" value={dashboard.team?.active || 0} detail={`${dashboard.team?.suspended || 0} suspensos`} /><Card label="Licenças" value={dashboard.seats?.active || 0} detail={`${dashboard.seats?.available || 0} disponíveis`} /><Card label="Convites pendentes" value={dashboard.invitations?.pending || 0} detail={`${dashboard.invitations?.pendingClinical || 0} clínicos`} /></> : <><Card label="Meus pacientes" value={dashboard.myPatients?.active || 0} /><Card label="Primary" value={dashboard.myPatients?.primary || 0} /><Card label="Secondary" value={dashboard.myPatients?.secondary || 0} /><Card label="Consultor" value={dashboard.myPatients?.consultant || 0} /></>}
      </section>
      <section className="rounded-2xl border border-brand-border bg-white p-5 shadow-sm"><div className="flex flex-wrap items-center justify-between gap-3"><div><h2 className="text-lg font-semibold text-brand-text">Status da clínica</h2><p className="mt-1 text-sm text-brand-text-muted">Acesso clínico: {org.clinicalAccessEnabled ? "habilitado" : "não habilitado"} · Operação: {entitlement}</p></div><span className="inline-flex items-center gap-2 rounded-full bg-emerald-50 px-3 py-1.5 text-sm font-semibold text-emerald-700"><ShieldCheck size={16} /> {org.membershipRole}</span></div>{admin && <p className="mt-4 text-sm text-brand-text-muted">Assignments ativos: {dashboard.assignments?.active || 0} · Primary {dashboard.assignments?.primary || 0} · Secondary {dashboard.assignments?.secondary || 0} · Consultores {dashboard.assignments?.consultant || 0}</p>}</section>
