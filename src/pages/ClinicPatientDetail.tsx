...
export default function ClinicPatientDetail() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (isManager && organizationId) setMembers((await fetchClinicTeam(session.access_token, organizationId)).filter((member) => member.status === "active" && member.clinical_access_enabled));
    } catch { setError("Não foi possível carregar este paciente ou você não possui acesso."); }
