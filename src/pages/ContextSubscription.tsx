...
export default function ContextSubscription() {
  ...
  return <div className="space-y-6">
    ...
    <section className="rounded-2xl border border-brand-border bg-white p-6 shadow-sm">
      <div className="flex items-start gap-3"><ShieldCheck className="mt-0.5 shrink-0 text-emerald-600" size={21} /><div><h2 className="text-lg font-semibold text-brand-text">Licença vinculada à clínica</h2><p className="mt-2 text-sm leading-6 text-brand-text-muted">A disponibilidade dos recursos clínicos é administrada pela organização. Para dúvidas sobre sua licença, fale com o proprietário ou gestor.</p></div></div>
      <p className="mt-5 rounded-xl bg-brand-bg px-4 py-3 text-sm font-semibold text-brand-text">{organization.tradeName || organization.name} · {organization.licenseActive ? "Licença ativa" : "Sem licença clínica"}</p>
