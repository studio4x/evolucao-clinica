...
export default function ClinicBilling() {
  ...
  async function runSeatChange() {
    if (!canManage || !status?.subscription) return;
  ...
  async function runCancel() {
    if (!canManage || !status?.subscription || status.subscription.cancel_at_period_end) return;
  ...
  return <div className="space-y-6">
    ...
    {loading ? <div className="flex items-center gap-2 rounded-2xl border border-brand-border bg-white p-6 text-sm text-brand-text-muted"><Loader2 className="animate-spin" size={18} /> Carregando status de cobrança…</div> : <>
      ...
      {status?.subscription ? <section className="space-y-5 rounded-2xl border border-brand-border bg-white p-5 shadow-sm">
        ...
        {canManage && <div className="flex flex-wrap items-end gap-3 border-t border-brand-border pt-5"><label className="text-sm font-semibold text-brand-text">Licenças<input className="mt-1 block w-28 rounded-xl border border-brand-border px-3 py-2" type="number" min={status.subscription.minimum_contracted_seats} value={seats} onChange={(event) => setSeats(Number(event.target.value) || status.subscription!.minimum_contracted_seats)} /></label><button disabled={busy || seats === status.subscription.contracted_seats} onClick={() => void runSeatChange()} className="rounded-xl bg-brand-primary px-4 py-2.5 text-sm font-bold text-white disabled:opacity-50">{busy ? "Processando…" : "Atualizar licenças"}</button>{!status.subscription.cancel_at_period_end && <button disabled={busy} onClick={() => void runCancel()} className="rounded-xl border border-red-200 px-4 py-2.5 text-sm font-bold text-red-700 disabled:opacity-50">Programar cancelamento</button>}</div>}
