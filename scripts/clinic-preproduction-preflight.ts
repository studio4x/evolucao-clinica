...
const report = { ref, projectId: project.id, teamId, deployment: { id: deployment.id, state: deployment.readyState, sha: deployment.meta?.githubCommitSha, branch: deployment.meta?.githubCommitRef }, health: { status: healthResponse.status, redirected: healthResponse.status >= 300, json: !!health, body: health }, flags: Object.fromEntries(flags.map((key) => [key, stagingEnv[key] ?? 'UNSET'])), baseline, stripe: { accountId: stripeAccount.id, keyMode: 'test' }, brevo: { host: stagingEnv.CLINIC_INVITATION_SMTP_HOST || '', credentialConfigured: !!stagingEnv.CLINIC_INVITATION_SMTP_PASS, fromConfigured: !!stagingEnv.CLINIC_INVITATION_SMTP_FROM, trackingDisabled: stagingEnv.CLINIC_INVITATION_SMTP_TRACKING_DISABLED, deliveryEnabled: stagingEnv.CLINIC_INVITATION_DELIVERY_ENABLED } };
