...
const flags = ['CLINIC_FEATURE_ENABLED', 'VITE_CLINIC_FEATURE_ENABLED', 'CLINIC_INVITATION_DELIVERY_ENABLED', 'CLINIC_INVITATION_SHARED_PROVIDER_ACCEPTED', 'CLINIC_BILLING_ENABLED', 'GOOGLE_INTEGRATIONS_ENABLED', 'VITE_GOOGLE_INTEGRATIONS_ENABLED'];
const report = { ref, projectId: project.id, teamId, deployment: { id: deployment.id, state: deployment.readyState, sha: deployment.meta?.githubCommitSha, branch: deployment.meta?.githubCommitRef }, health: { status: healthResponse.status, redirected: healthResponse.status >= 300, json: !!health, body: health }, flags: Object.fromEntries(flags.map((key) => [key, gateValue(key)])), baseline, stripe: { accountId: stripeAccount.id, keyMode: 'test' }, brevo: { host: stagingEnv.CLINIC_INVITATION_SMTP_HOST || '', credentialConfigured: !!stagingEnv.CLINIC_INVITATION_SMTP_PASS, fromConfigured: !!stagingEnv.CLINIC_INVITATION_SMTP_FROM, sharedProviderAccepted: stagingEnv.CLINIC_INVITATION_SHARED_PROVIDER_ACCEPTED === 'true', smtpTransportReady: createInvitationTransport(stagingEnv).ready, trackingDisabled: stagingEnv.CLINIC_INVITATION_SMTP_TRACKING_DISABLED, deliveryEnabled: gateValue('CLINIC_INVITATION_DELIVERY_ENABLED') } };
