...
export function evaluateBrevoPreflight(input: { smtpAuthentication: boolean; senderConfigured: boolean; directFragment: boolean; localTrackingDeclaration: boolean; deliveryGate: string; provider?: ProviderEvidence }) {
  ...
  const complete = input.smtpAuthentication && input.senderConfigured && senderProviderVerified === true && providerScopeIsolated === true && providerScope === 'ISOLATED' && strictTrackingDisable === 'VERIFIED' && input.directFragment && input.localTrackingDeclaration && input.deliveryGate === 'OFF';
