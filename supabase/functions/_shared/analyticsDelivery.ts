...
const ALLOWED_PARAMS = new Set(["transaction_id", "value", "currency", "plan_id", "plan_name", "payment_provider", "session_id", "session_number", "engagement_time_msec"]);
...
export function buildMeasurementPayload(input: { eventName: AnalyticsEventName; userId: string; params: Record<string, string | number>; attribution: Attribution; occurredAt: string }) {
  ...
  const params: Record<string, string | number> = { ...input.params, engagement_time_msec: 1 };
