-- Expand the privileged account-destruction RPC for session control data.
-- Normal edits remain protected; only this service-role-only transaction bypasses
-- user triggers while it removes one professional's complete clinical dataset.
