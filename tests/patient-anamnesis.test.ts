...
const supportTicketsSource = readFileSync('src/pages/SupportTickets.tsx', 'utf8');
const supportTicketModalSource = readFileSync('src/components/support/SupportTicketModal.tsx', 'utf8');
...
assert.match(anamnesisPageSource, /ANAMNESIS_SUPPORT_HREF/);
assert.match(anamnesisPageSource, /supportHref={ANAMNESIS_SUPPORT_HREF}/);
assert.match(supportTicketsSource, /useSearchParams/);
assert.match(supportTicketsSource, /searchParams.get\('new'\)/);
assert.match(supportTicketsSource, /initialDescription={ticketDraft\?.description}/);
assert.match(supportTicketModalSource, /initialSubject\?: string/);
assert.match(supportTicketModalSource, /setDescription\(initialDescription\)/);
...
assert.match(anamnesisModelsSource, /FeatureGuideButton/);
assert.match(anamnesisModelsSource, /FeatureGuideModal/);
assert.match(anamnesisModelsSource, /Como funcionam os modelos de Anamnese\?/);
assert.match(anamnesisModelsSource, /As Anamneses já registradas preservam suas informações/);
assert.match(anamnesisModelsSource, /md:hidden/);
assert.match(anamnesisBuilderSource, /FeatureGuideButton/);
assert.match(anamnesisBuilderSource, /FeatureGuideModal/);
assert.match(anamnesisBuilderSource, /Como criar seu modelo de Anamnese\?/);
assert.match(anamnesisBuilderSource, /sourceTemplate \? 'Você está usando um modelo existente/);
