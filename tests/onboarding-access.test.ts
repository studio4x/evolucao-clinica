...
assert.match(onboardingSource, /const handleExitOnboarding = async \(\) =>/);
assert.match(onboardingSource, /await deferOnboarding\(user\.id, 'intro'\)/);
assert.match(onboardingSource, /data-testid="onboarding-exit-to-app"/);
assert.match(onboardingSource, /Sair e voltar ao aplicativo/);
