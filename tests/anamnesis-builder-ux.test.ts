...
assert.match(models, /const openBuilder = \(path: string\) => navigate\(path, \{ state: \{ from: returnTo \} \}\)/);
assert.equal((models.match(/openBuilder\(`/g) || []).length, 3);
assert.match(models, /openBuilder\('\/painel\/anamnesis\/modelos\/novo'\)/);
...
assert.match(builder, /ActionModal/);
assert.match(builder, /Salve o modelo para adicionar campos/);
assert.match(builder, /Antes de adicionar novos campos, salve este modelo/);
assert.match(builder, /confirmLabel="Salvar modelo"/);
assert.match(builder, /cancelLabel="Agora não"/);
assert.match(builder, /setSaveBeforeAddFieldOpen\(true\)/);
assert.match(builder, /if \(!editingTemplate\)/);
assert.match(builder, /const appendFieldToSchema/);
assert.match(builder, /continueWithAddFieldSectionId/);
assert.match(builder, /const savedTemplate = wasEditing/);
assert.match(builder, /savedTemplate\.id/);
assert.match(builder, /navigate\(`\/painel\/anamnesis\/modelos\/\$\{savedTemplate\.id\}\/editar`/);
assert.match(builder, /savingRef\.current/);
assert.match(builder, /return false;/);
assert.match(builder, /setMaterializedTemplateId\(savedTemplate\.id\)/);
assert.match(builder, /sourceTemplateId: sourceTemplate\?\.kind === 'system'/);
...
assert.match(builder, /useLocation/);
assert.match(builder, /requestedReturnTo.*startsWith\('\/painel\/'\)/s);
assert.match(builder, /returnToModels = \(\) => navigate\('\/painel\/anamnesis\/modelos', \{ state: \{ from: returnTo \} \}\)/);
assert.equal((builder.match(/returnToModels\(\);/g) || []).length, 2);
