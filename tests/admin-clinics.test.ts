...
assert.doesNotMatch(migration, /max\s*\(\s*m\.professional_id\s*\)/i);
assert.match(migration, /owner_count\.owner_count/);
assert.match(migration, /ORDER BY m\.id\s+LIMIT 1/);
