...
console.log("Conectando ao banco de dados Supabase para adicionar quick_notes...");
...
async function run() {
  try {
    ...
    const migrationPath = path.join(__dirname, '../supabase/migrations/20260622230000_add_quick_notes_to_patients.sql');
    console.log(`Lendo migração de: ${migrationPath}`);
    const sql = fs.readFileSync(migrationPath, 'utf8');

    console.log("Executando SQL...");
    await client.query(sql);
    console.log("Migração aplicada com sucesso (Coluna quick_notes adicionada)!");
  } catch (err) {
