...
console.log("Conectando ao banco de dados Supabase para aplicar migração de Política RLS de UPDATE...");
...
async function run() {
  try {
    ...
    const migrationPath = path.join(__dirname, '../supabase/migrations/20260629240000_add_update_policy_to_patient_reports.sql');
    console.log(`Lendo migração de: ${migrationPath}`);
    const sql = fs.readFileSync(migrationPath, 'utf8');

    console.log("Executando SQL...");
    await client.query(sql);
    console.log("Política RLS de UPDATE aplicada com sucesso!");
  } catch (err) {
