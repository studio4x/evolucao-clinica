...
export const INVITATION_LANDING_SCRIPT = `(async()=>{let token=new URLSearchParams(location.hash.slice(1)).get('invite');history.replaceState(null,'','/convite-clinica');try{if(!token||!/^[a-f0-9]{64}$/.test(token))throw 0;const response=await fetch('/api/clinic/invitations/handoff',{method:'POST',credentials:'same-origin',headers:{'Content-Type':'application/json'},body:JSON.stringify({token}),cache:'no-store',referrerPolicy:'no-referrer'});token=null;if(!response.ok)throw 0;location.replace('/painel/convite-clinica');}catch{token=null;document.getElementById('status').textContent='Convite inválido, expirado ou indisponível. Solicite um novo convite à clínica.';}})();`;
export const INVITATION_LANDING_HTML = `<!doctype html><html lang="pt-BR"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="robots" content="noindex,nofollow"><meta name="referrer" content="no-referrer"><title>Convite — Evolução Clínica</title><script defer src="/api/clinic/invitations/landing.js"></script></head><body style="font-family:Arial;color:#105576;padding:32px"><h1>Evolução Clínica</h1><p id="status">Preparando seu convite com segurança…</p></body></html>`;
