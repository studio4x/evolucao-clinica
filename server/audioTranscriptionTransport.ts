  ...
  try {
    ...
    if (useInline) {
      ...
      const geminiFile = await ai.files.upload({
        file: new Blob([new Uint8Array(input.audioBuffer)], { type: input.mimeType }),
      ...
      audioPart = createPartFromUri(geminiFile.uri, geminiFile.mimeType || input.mimeType) as Record<string, unknown>;
    }
    ...
    result = { transcription, method, cleanupSucceeded: true, usageMetadata: response.usageMetadata };
    return result;
  } finally {
    if (geminiFileName) {
      try {
